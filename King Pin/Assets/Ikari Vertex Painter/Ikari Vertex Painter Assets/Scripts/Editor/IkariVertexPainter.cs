...
public class IkariVertexPainter : EditorWindow
{
	...
	{
		EditorWindow.GetWindow(typeof(IkariVertexPainter));
	}
	...
	void OnEnable()
	{
		...
		folderPath = GetScriptPath().Remove(GetScriptPath().Length - 36);
										...
										if (objectProperties.foldout)
										{
											...
											if (!selectedTransform.GetComponent<ObjectProperties>().showingVertex)
											{
												if (GUILayout.Button("Show Vertex Colors", GUILayout.MaxWidth(position.width - offsetButton)))
												{
													...
													selectedTransform.GetComponent<Renderer>().sharedMaterial = new Material(Shader.Find("Ikari Vertex Painter/Debug/Vertex Colors"));
					...
					if(selectedTransform.GetComponent<MeshFilter>() != null){
						if(selectedTransform.GetComponent<ObjectProperties>() == null){
							ObjectProperties objectPropertiesScript = selectedTransform.AddComponent<ObjectProperties>();
								...
								Selection.activeTransform.GetComponent<Renderer>().sharedMaterial = new Material(Shader.Find("Ikari Vertex Painter/Debug/Vertex Colors"));
