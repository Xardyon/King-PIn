...
namespace HutongGames.PlayMaker.Actions
{
		...
		public string descriptionArea = "Minimum Dotween version requirement: v1.0.770";
		...
		private string debugString = " ...mmm did 'DoTween' work ?? ";
		...
		public override void Reset ()
		{
			...
			if (duration.Value == 0) {
				Debug.LogWarning ("!!! Duration cannot be 0 !!! ");
				Finish();
			}
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("!!! Loop Time is set but no 'Loop Type' is selected !!! ");
				break;
			...
			case updateType.Normal:
				setFinal.SetUpdate(UpdateType.Normal);
				break;
				
			case updateType.Fixed:
				setFinal.SetUpdate(UpdateType.Fixed);
				break;
				
			case updateType.Late:
				setFinal.SetUpdate(UpdateType.Late);
				break;
			...
			switch(dotweenTypeSelect){
				...
				if (shakeBy.IsNone)
				_target.transform.DOShakePosition(duration.Value,to.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
					else
				_target.transform.DOShakePosition(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
				break;
					
				case doTweenType.DoShakeRotation:
				if (shakeBy.IsNone)
					_target.transform.DOShakeRotation(duration.Value,to.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					else
					_target.transform.DOShakeRotation(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoShakeScale:
				if (shakeBy.IsNone)
					_target.transform.DOShakeScale(duration.Value, to.Value, vibrato.Value, elasticity.Value ).SetAs(setFinal);
					else
					_target.transform.DOShakeScale(duration.Value, shakeBy.Value, vibrato.Value, elasticity.Value ).SetAs(setFinal);
					break;
			...
			{
			if (DotweenDone.Value != true && startEvent == null && (loopTypeSelect == loopType.None)) Debug.LogWarning (debugString);
			}
