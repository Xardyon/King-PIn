...
namespace HutongGames.PlayMaker.Actions
{
		...
		public override void Reset ()
		{
			...
			switch(dotweenTypeSelect){
				...
				if (shakeBy.IsNone)
					_target.GetComponent<Camera>().DOShakePosition(duration.Value,to.Value,vibrato.Value, elasticity.Value).SetAs(setFinal);
					else
					_target.GetComponent<Camera>().DOShakePosition(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value).SetAs(setFinal);
				break;
					
				case doTweenType.DoShakeRotation:
				if (shakeBy.IsNone)
					_target.GetComponent<Camera>().DOShakeRotation(duration.Value,to.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					else
					_target.GetComponent<Camera>().DOShakeRotation(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					break;
