...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class GameObjectShake : FsmStateAction
	{
		...
		{
			if (isCamera.Value == false){doSetup();}
		}


		public override void OnLateUpdate()
		{
			if (isCamera.Value == true){doSetup();}
		}
			...
			if (loop.Value == true)
			{
				...
				if (shakeTime.Value <= 0f)
				{
					...
					if (exitOnFinish.Value == true){
						...
						Fsm.Event(exit);
						if (shakeTime.Value <= 0.0f) Finish();
					}
		...
		void DoObjShake()
		{
			...
			if (shakeTime.Value > 0f || loop.Value == true){
		...
		void DoObjShakerotation(){
			...
			if (animationCurvebool.Value == true){
				objTransform.localRotation = Quaternion.Lerp(newPosRot,objTransform.localRotation, lerpCurve.curve.Evaluate(shakeTime.Value));
			}
			
			else if (lerpOn.Value == true){
		...
		void DoObjShakeLerp()
		{
			...
			if (animationCurvebool.Value == true){
				objTransform.localPosition = Vector3.Lerp(objTransform.localPosition, newPos , lerpCurve.curve.Evaluate(shakeTime.Value));
			}
