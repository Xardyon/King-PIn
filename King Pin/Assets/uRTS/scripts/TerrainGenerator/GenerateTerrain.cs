...
public class GenerateTerrain : MonoBehaviour {
	...
	public int nTilesRadius = 4;
	
	public bool useProceduralGeneration = true;
	
	private TerrainChunkSettings tcs;
...
//	private Material TerrainMaterial;
	...
	public List<Vector2> ignoredTiles;
	public Terrain manualTerrain;
	
	[HideInInspector] public GameObject masterTerrain;
	...
	//    if(clonedTerrainData==null){
		
	//    }
	    CleanUp();
	    
	    camTransf = Camera.main.gameObject.transform;
	    masterTerrain = new GameObject("Procedural terrain");
		Test2();
		Starter();
		
		foreach(GameObject go in tcs.terrainGos){
			go.transform.SetParent(masterTerrain.transform);
		}
		
//		Debug.Log(tcs.terrainGos.Count);
		
		masterTerrain.transform.position = masterTerrain.transform.position + new Vector3(0f,verticalShift,0f);
		
	}
	
	
	public void CleanUp(){
		DestroyImmediate(masterTerrain);
		if(backupTerrain != null){
			TerrainData clonedTerrainData = (TerrainData) Instantiate(backupTerrain.terrainData);
			manualTerrain.terrainData = clonedTerrainData;
		}
	}
...
// 	    tcs = new TerrainChunkSettings(129, 129, 100, 40);
// 	    tcs.Settings = tcs;
// 	    tcs.Test();
// 	    tcs.Updater();
//		Test2();
	...
	{	
		...
		var Settings = new TerrainChunkSettings(heightmapResolution, alphamapResolution, length, height, FlatTexture, SteepTexture, null);
	...
	public void Test2(){
		tcs = new TerrainChunkSettings(heightmapResolution, alphamapResolution, length, height, FlatTexture, SteepTexture, null);
		
		TerrainChunkSettings mtcs = new TerrainChunkSettings(manualTerrain.terrainData.heightmapResolution, alphamapResolution, length, height, FlatTexture, SteepTexture, null);
		
		tcs.verticalShift = verticalShift;
		mtcs.verticalShift = verticalShift;
		
		tcs.treeName = treeName;
		tcs.numberOfPlants = numberOfPlants;
		tcs.minimumCriticalDistance = minimumCriticalDistance;
		
		tcs.Settings = tcs;
		tcs.Test();
		
		if(useProceduralGeneration == false){
		    var Settings = tcs;
		    //new TerrainChunkSettings(heightmapResolution, alphamapResolution, length, height, FlatTexture, SteepTexture, null);
		    
		    Settings.treeName = treeName;
			Settings.numberOfPlants = numberOfPlants;
			Settings.minimumCriticalDistance = minimumCriticalDistance;
		    
			var NoiseProvider = new NoiseProvider();
			
			float[,] hMap = null;
			
			TerrainChunk chnk = null;
			
			for (int i = -nTilesRadius; i < nTilesRadius; i++){
        		for (int j = -nTilesRadius; j < nTilesRadius; j++){
        			bool isAllowed = true;
        			for(int k=0;k<ignoredTiles.Count;k++){
        				if((i==(int)ignoredTiles[k].x)&&(j==(int)ignoredTiles[k].y)){
        					isAllowed = false;
        				}
        			}
        			if(isAllowed == true){
        			    ...
        			    chnk = new TerrainChunk(mtcs, NoiseProvider, i, j);
        				hMap = chnk.GetHeightmap(0f);
        			}
        		}
        	}
        	
        	CorrectManualTerrainEdges(hMap);
        	if(chnk != null){
        		chnk.ApplyTextures(manualTerrain.terrainData);
        	}
	...
	void CorrectManualTerrainEdges(float[,] hMap){
		int resol = manualTerrain.terrainData.heightmapResolution;
		var newHeightmap = new float[resol, resol];
		
		float[,] hMap2 = new float[resol, resol];
		
		for (var zRes = 0; zRes < resol; zRes++)
		{
			for (var xRes = 0; xRes < resol; xRes++)
			{
				hMap2[zRes, xRes] = (hMap[zRes, xRes]*height + verticalShift)/manualTerrain.terrainData.size.y;
			}
		...
		for (var zRes = 0; zRes < resol; zRes++)
		{
			for (var xRes = 0; xRes < resol; xRes++)
			{
			//	var xCoordinate = X + (float)xRes / (resol - 1);
			//	var zCoordinate = Z + (float)zRes / (resol - 1);
				float coef = 0.5f;
				
				float distX = Mathf.Abs(0.5f*resol - xRes);
				float distZ = Mathf.Abs(0.5f*resol - zRes);
				
		//		dist = Mathf.Max(distX, distZ)/(0.5f*resol);
				
				dist = Mathf.Sqrt(distX*distX+distZ*distZ)/(0.5f*resol);
		...
		//		print(dist);
				...
				newHeightmap[zRes, xRes] = coef*hMap2[zRes, xRes] + (1f-coef)*(manualTerrain.terrainData.GetHeight(xRes,zRes))/manualTerrain.terrainData.size.y;
			}
		//	print(dist);
		}
		
		manualTerrain.terrainData.SetHeights(0, 0, newHeightmap);
	...
	void Update(){
		if(tcs != null){
			if(useProceduralGeneration == true){
				tcs.Updater(camTransf.position, nTilesRadius);
			}
		}
		else{
//			print("aa");
...
namespace TerrainGenerator
{
	...
	public class TerrainChunkSettings
	{
		...
		public void Updater(Vector3 pos, int nTilesRadius){
			Cache.Update();
			UpdateTerrain(pos, nTilesRadius);
		}
...
//			Debug.Log(positionsToGenerate[0].X);
	...
	{	
		...
		public TerrainChunkSettings Settings { get; set; }
...
// 		public List<float> minimumCriticalDistance;
		...
		public void CreateTerrain()
		{
			...
			var heightmap = GetHeightmap(0f);
			treeMap = GetHeightmap(1f);
...
//			Debug.Log(Z);
	...
	//		var heightmap2 = GetHeightmap(500f);
			CreateVegetation();
			
	//		newTerrainGameObject.GetComponent<TerrainCollider>().terrainData = terrainData;
			
		}
		
		
		public void ApplyTextures(TerrainData terrainData)
        {
            ...
            flatSplat.tileSize = new Vector2(2f,2f);
            steepSplat.tileSize = new Vector2(2f,2f);
            ...
            for (var zRes = 0; zRes < terrainData.alphamapHeight; zRes++)
            {
                for (var xRes = 0; xRes < terrainData.alphamapWidth; xRes++)
                {
                    ...
                    steepnessNormalized = steepness / (4.9f*90f*Settings.Height/Settings.Length);
        ...
        private void CreateVegetation(){
        	...
        	vegetation.terrainData = Terrain.terrainData;
        	
        	vegetation.chunk = this;
        	vegetation.treeName = Settings.treeName;
        	vegetation.numberOfPlants = Settings.numberOfPlants;
        	vegetation.minimumCriticalDistance = Settings.minimumCriticalDistance;
        	
        	vegetation.pn = treeMap;
		...
		public float[,] GetHeightmap(float yCoordinate)
		{	
			...
			for (var zRes = 0; zRes < Settings.HeightmapResolution; zRes++)
			{
				for (var xRes = 0; xRes < Settings.HeightmapResolution; xRes++)
				{
					...
					heightmap[zRes, xRes] = NoiseProvider.GetValue(xCoordinate, yCoordinate, zCoordinate);
				}
			}
			...
			lock (HeightmapThreadLockObject)
			{
				...
				for (var zRes = 0; zRes < Settings.HeightmapResolution; zRes++)
				{
					for (var xRes = 0; xRes < Settings.HeightmapResolution; xRes++)
					{
						...
						heightmap[zRes, xRes] = NoiseProvider.GetValue(xCoordinate, 0f, zCoordinate);
					}
				}
...
// http://libnoise.sourceforge.net/ noise libraries used for terrain generation noises
...
namespace LibNoise
{
	...
	public class NoiseProvider : INoiseProvider
	{
		...
		{
			return (float)(PerlinNoiseGenerator.GetValue(x, y, z) / 2f) + 0.5f;
		}
	}
  ...
  //          XXHash randomHashObject = new XXHash((int)(i*uint.MaxValue));
            
  //          var cvg = randomHashObject.Value(ix,iz);
            var xvg = Randoms[(i << 2)];
...
//            return cvg;
 ...
 //           XXHash randomHashObject = new XXHash(12345);
            
   //         value = randomHashObject.GetHash(x,z);
          ...
          //      var signal = (double)randomHashObject.GetHash((int)(nx* uint.MaxValue),(int)(nz* uint.MaxValue))/uint.MaxValue;
