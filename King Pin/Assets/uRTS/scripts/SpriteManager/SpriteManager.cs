...
// 	private int rows = 32;
	...
	protected SpriteM[] sprites;				// Array of all sprites (the offset of the vertices corresponding to each sprite should be found simply by taking the sprite's index * 4 (4 verts per sprite).
...
// 	protected static List<MeshFilter> meshFilterList = new List<MeshFilter>();
// 	protected static List<MeshRenderer> meshRendererList = new List<MeshRenderer>();
	...
	// Working vars:
	protected int i;
	protected SpriteM tempSprite = null;
	protected float animTimeElapsed;
	
	[HideInInspector] public bool smEnabled = true;
	
//	private int iUpdate = 0;
//	private int nUpdates = 15;
...
/*		for(int j=0;j<columns;j++){
			for(int i=0;i<rows;i++){
				...
				string modelName = "knight";
				string animationName = "Knight_Walk";
			    
				string matfilePath = "Assets" + "/mat/"+animationName+"/";
				string matfileName = modelName+"_"+animationName+"_"+jj+"_"+ii+".mat";
				materials.Add(Resources.LoadAssetAtPath<Material>(matfilePath+matfileName));
			}
		}*/
	}
	
	public void ChangeMaterial(int mId)
	{
	//	meshRenderer.renderer.material = materials[mId];
	}
 
	// Allocates initial arrays
	protected void InitArrays()
	{
		sprites = new SpriteM[1];
		sprites[0] = new SpriteM();
	...
	protected int EnlargeArrays(int count)
	{
		...
		// Resize sprite array:
		SpriteM[] tempSprites = sprites;
		sprites = new SpriteM[sprites.Length + count];
	...
	public SpriteM AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, bool billboarded)
	{
	...
	public SpriteM AddSprite(GameObject client, float width, float height, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, Vector3 offset, bool billboarded)
	{
	...
	// offset - the sprite will be offseted by the specified Vector3 relative to the client transform
	public SpriteM AddSprite(GameObject client, float width, float height, Vector2 lowerLeftUV, Vector2 UVDimensions, Vector3 offset, bool billboarded)
	{
		int spriteIndex;
	...
	Vector3 CalculateNormal( SpriteM sprite )
	{
	...
	public void SetBillboarded(SpriteM sprite)
	{
	...
	public void RemoveSprite(SpriteM sprite)
	{
	...
	public void HideSprite(SpriteM sprite)
	{
		// Remove the sprite from the billboarded list
	...
	public void ShowSprite(SpriteM sprite)
	{
	...
	public void MoveToFront(SpriteM s)
	{
	...
	public void MoveToBack(SpriteM s)
	{
		int[] indices = new int[6];
		int offset = spriteDrawOrder.IndexOf(s) * 6;
	...
	public void MoveInfrontOf(SpriteM toMove, SpriteM reference)
	{
		int[] indices = new int[6];
	...
	public void MoveBehind(SpriteM toMove, SpriteM reference)
	{
		int[] indices = new int[6];
		int offset = spriteDrawOrder.IndexOf(toMove) * 6;
		int refOffset = spriteDrawOrder.IndexOf(reference) * 6;
	...
	public void AnimateSprite(SpriteM s)
	{
		// Add this sprite to our playingAnimation list:
		playingAnimations.Add(s);
	}
 
	public void StopAnimation(SpriteM s)
	{
		playingAnimations.Remove(s);
	}
 
	public SpriteM GetSprite(int i)
	{
		if (i < sprites.Length)
			return sprites[i];
		else
	...
	public void Transform(SpriteM sprite)
	{
		sprite.Transform();
	...
	public void TransformBillboarded(SpriteM sprite)
	{
	//	Vector3 posOffset = new Vector3(0f,9f,0f);
	...
	public void UpdateUV(SpriteM sprite)
	{
	...
	public void UpdateColors(SpriteM sprite)
	{
